      ...
      <div class="card full-height">
        <div class="card-header">
          <div class="card-head-row">
            <h1 class="card-title">Tambah penjualan</h1>
            ...
            <div class="form-group classAdded mb-2">
              <div class="row">
                <div class="col-md-6">
                  ...
                  <select name="id_obat[]" id="id_obat" class="form-control selectobat">
                    ...
                    <option value="{{ $row->id }}">{{ $row->nama_obat }} / {{ $row->jenis->nama_jenis }} | Rp.
                      {{ number_format($row->harga_jual) }} | Sisa Stok : {{ $row->stok }}</option>
                    @else
                    <option value="{{ $row->id }}">{{ $row->nama_obat }} | Rp. {{ number_format($row->harga_jual) }} |
                      Sisa Stok : {{ $row->stok }}</option>
                    @endif
                    @endforeach
                  </select>
                </div>
                ...
                <div class="col-md-1" style="margin-top: 1.8rem">
                  <a class="btn text-white addClass" style="background-color: #a979a8;"
                    onmouseover="this.style.backgroundColor='#6c5576';"
                    onmouseout="this.style.backgroundColor='#a979a8';" href="javascript:void(0)">+</a>
                </div>
              </div>
              ...
              $('form').on('click', '.addClass', function() {
                var form = "<div class='form-group classAdded'>" +
                  ...
                  "</div>" +
                  "<div class='col-md-1' style='margin-top: 1.8rem'>" +
                  "<a class='btn btn-secondary removeClass' href='javascript:void(0)'>-</a>" +
                  "</div>" +
                  "</div>" +
                  "</div>"
